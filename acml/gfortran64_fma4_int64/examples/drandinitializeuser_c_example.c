...
void whini(long *genid, long *subid, long *seed, long *lseed,
           long *state, long *lstate, long *info);
void whgen(long *n, long *state, double *x, long *info);
...
#define MN 12
  const long mn=MN;
  double a, b;
  long i, info, lseed, lstate, n;
  double x[MN];
  long seed[MSEED], state[MSTATE];
           ...
           long *state, long *lstate, long *info)
{
  ...
  */
  const long eseed=4, estate=25;
  const long magic=54321;
  long i;
...
void whgen(long *n, long *state, double *x, long *info)
{
  ...
  */
  const long magic=54321;
  double w;
  long i, imagt, imagx, imagy, imagz, imodt, imodx,
