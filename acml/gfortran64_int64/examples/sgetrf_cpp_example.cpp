...
/*
 * ACML version 2.7 Copyright AMD,NAG 2003
 */
...
#define B(I,J) b[((J)-1)*ldb+(I)-1]
  ...
  cout << "ACML example: solution of linear equations using sgetrf/sgetrs" << endl;
  cout << "--------------------------------------------------------------" << endl;
  cout << endl;
  ...
  cout << endl;
  cout << "Right-hand-side matrix B:" << endl;
  for (i = 1; i <= n; i++)
    {
      for (j = 1; j <= nrhs; j++)
        printf("%8.4f ", B(i,j));
  ...
  if (info == 0)
    {
      ...
      /* Print solution */
      cout << "Solution matrix X of equations A*X = B:" << endl;
      for (i = 1; i <= n; i++)
        {
          for (j = 1; j <= nrhs; j++)
            printf("%8.4f ", B(i,j));
    ...
    cout << "The factor U of matrix A is singular" << endl;
